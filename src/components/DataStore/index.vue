    ...
    <bk-navigation :header-title="nav.id" :side-title="nav.title" :default-open="false"
      ...
      <template slot="header">
        <div class="monitor-navigation-header">
          ...
          <div v-else class="header-title">
            <span class="header-title-icon">
              <svg class="icon"
                ...
                <path d="M416 480h320v64H416l96 96-48 48-176-176 176-176 48 48-96 96z" p-id="4757">
                </path>
              </svg>
            </span>
            {{nav.id}}
          ...
          <bk-select class="header-select" :class="{ 'is-left': curNav.nav === 'left-right' }"
            ...
            <bk-option v-for="option in header.selectList" :key="option.id" :id="option.id"
              :name="option.name">
            </bk-option>
          </bk-select>
      ...
      <template slot="menu">
        <bk-navigation-menu ref="menu" @select="handleSelect" :default-active="nav.id"
          ...
          <bk-navigation-menu-item v-for="item in nav.list" :key="item.name"
            :has-child="item.children && !!item.children.length" :group="item.group" :icon="item.icon"
            :disabled="item.disabled" :url="item.url" :id="item.name">
            <span>{{item.name}}</span>
            <div slot="child">
              <bk-navigation-menu-item :key="child.name" v-for="child in item.children" :id="child.name"
                :disabled="child.disabled" :icon="child.icon" :default-active="child.active">
